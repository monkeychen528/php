...
$t_sql = "SELECT COUNT(1),s.$day FROM `allstore` a JOIN `store_information` s ON s.`sid` =a.`sid`
WHERE a.`type` LIKE '$type'"; //原本select後面接星號現在改為count(形式參數)，count會去找出資料總比數用()參數代替
            ...
            <nav aria-label="Page navigation example" style="margin-top: .5rem;">
                <ul class="pagination">
                    <li class="page-item d-flex">
                        <a class="page-link " href="?page=<?= $page - 15 ?>&day=<?= $day ?>&type=<?= $type ?>">
                        <i class="fas fa-angle-double-left"></i>
                        </a>
                        <a class="page-link " href="?page=<?= $page - 1 ?>&day=<?= $day ?>&type=<?= $type ?>">
                    ...
                    for ($i = $p_start; $i <= $p_end; $i++) :
                        ...
                        <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                            <a class="page-link " href="?page=<?= $i ?>&day=<?= $day ?>&type=<?= $type ?>"><?= $i ?></a></li>
                        ...
                        <div class="card">
                            <tr>
                                ...
                                <div class="t_content">
                                    ...
                                    <td><?= htmlentities($r['新增活動權限']); ?>
                                        <a href="javascript:event_pre('<?= htmlentities($r['store-name']) ?>',false)">
                                        <i class="fas fa-times"></i></a>關閉
                                        <a href="javascript:event_pre('<?= htmlentities($r['store-name']) ?>',true)">
                                        <i class="fas fa-check"></i></a>允許
                                    </td>
                                    <td><?= htmlentities($r['商家上架']); ?>
                                        <a href="javascript:store_pre('<?= htmlentities($r['store-name']) ?>',false)">
                                        <i class="fas fa-times"></i></a>關閉
                                        <a href="javascript:store_pre('<?= htmlentities($r['store-name']) ?>',true)">
                                        <i class="fas fa-check"></i></a>上架
                                    </td>
                                </div>
...
<script>
    ...
    //--------活動權限
    function event_pre(store, value) {
        if(value){
            if (confirm(`確定要開啟${store}的活動功能嗎?`)) {
                location.href = 'bar_evt_pre_api.php?store-name=' + store + '&value=' + '允許'        
            }
        }
        else{
            if(confirm(`確定要關閉${store}的活動功能嗎?`)){
            location.href = 'bar_evt_pre_api.php?store-name=' + store + '&value=' + '不允許'  
            }      
        }
    };

    //-------店家權限
    function store_pre(store, value) {
        if(value){
            if (confirm(`確定要${store}更改為上架嗎?`)) {
                location.href = 'bar_store_pre_api.php?store-name=' + store + '&value=' + '允許'        
            }
        }
        else{
            if(confirm(`確定要${store}更改為下架嗎?`)){
            location.href = 'bar_store_pre_api.php?store-name=' + store + '&value=' + '不允許'  
            }      
        }
    };

    //--------ajax //這邊應該要改成頁數的ajax，目前只是單純的post，修正的話變成dom裡面的el標籤要empty再換成新的插入進去
